...
----------   ---   ---------------------------------------------------------
2019-07-01   rkr    Add version introduced
...
typedef enum {
    ...
    PSM_ERR_INVALID_INTERVAL,      /**< Invalid Interval time passed to the server */
...
typedef struct {
    ...
    uint32_t                psm_duration_in_secs;   /**< Time in seconds if the time_format_flag is set to PSM_TIME_IN_SECS. */
}psm_time_info_type;
 ...
 * }
 * PSM time interval structure.
 */
typedef struct  {
    psm_time_info_type start; /**< Start time of the Interval */
    psm_time_info_type end;   /**< End time of the Interval */
    psm_wakeup_type_e  psm_wakeup_type; /**< Next wake up from PSM mode is for
                                         measurement purpose or measurement and network access. */
}psm_time_interval_type;

/**
 * @versiontable{2.0,2.45,
 * Modem\_Features 1.2.0  &  Introduced. @tblendline
 * }
 * PSM time interval group structure.
 */
typedef struct  {
    int                      number_of_intervals; /**< Number of time intervals being passed in the structure */
    psm_time_interval_type*  intervals; /**< Array of time intervals */
}psm_time_interval_group_type;

/**
 * @versiontable{2.0,2.45,
 * Modem\_Features 1.2.0  &  Introduced. @tblendline
 * }
 * PSM information type with interval.
 */
typedef struct {
    int                    active_time_in_secs;  /**< Active time is the duration the PSM server
                                                      must wait before entering PSM mode.
                                                      The purpose of this time is to provide a
                                                      chance for the MTC server to react. */
    psm_time_interval_type  psm_time_info;       /**< PSM Interval time information. See #psm_time_interval_type. */
}psm_info_interval_type;
