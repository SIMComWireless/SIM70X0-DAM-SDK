 ...
 * --------   ---     ----------------------------------------------------------
 * 01/07/19   rkr     Add version introduced
 * 07/01/19   rkr     Add new QAPIs for LwM2M Usecases
...
/*
!! IMPORTANT NOTE: "MODEM FEATURE" VERSION CATEGORY.
*/
...
#define    TXM_QAPI_PSM_CLIENT_OPTML_INTRVL          TXM_QAPI_PSM_BASE + 8
#define    TXM_QAPI_PSM_CLIENT_ENTER_PSM_WITH_RANGE  TXM_QAPI_PSM_BASE + 9
...
#define    qapi_PSM_Client_Get_Optimal_Interval(a,b,c) ((qapi_PSM_Status_t) (_txm_module_system_call12)(TXM_QAPI_PSM_CLIENT_OPTML_INTRVL , (ULONG) a, (ULONG) b, (ULONG) c, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0))
#define    qapi_PSM_Client_Enter_Psm_With_Range(a,b) ((qapi_PSM_Status_t) (_txm_module_system_call12)(TXM_QAPI_PSM_CLIENT_ENTER_PSM_WITH_RANGE , (ULONG) a, (ULONG) b, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0))
...
/*============================================================================
                qapi_PSM_Client_Get_Optimal_Interval
============================================================================*/
/**
 * @versiontable{2.0,2.45,
 * Modem\_Features 1.2.0  &  Introduced. @tblendline
 * }
 *  @brief     Calculates the optimal range of time to vote for PSM from 
 *             a given set of intervals.
 *
 *  @details   Frequently, PSM Clients come up with different ranges of intervals 
 *             during which the device is needed to be online.
 *             This gives clients a way to rely on the PSM Daemonto calculate the optimal
 *             range out of the given set of intervals.
 *
 *  @param[out] optimal_interval Pointer to optmial interval. Memory must be allocated by
 *               client.
 *
 *  @param[in] client_id Client ID obtained during registration.
 *
 *  @param[in] interval_group Structure containing the array of intervals that clients
 *             want to get the optimal interval for.
 *
 *  @return  Returns QAPI_OK on success or a -ve error code on failure. \n
 *  - QAPI_ERR_PSM_INVALID_INTERVAL -- One of the following reasongs:
 *                         One or more intervals in the input have invalid format.
 *                         End of an interval is before the start.
 *                         Length of input set of intervals is invalid.
 *  - QAPI_ERR_PSM_WRONG_ARGUMENTS -- One or more of the arguments are NULL.
 *  - QAPI_ERR_PSM_APP_NOT_REGISTERED -- Invalid client ID.
 *  - QAPI_ERR_PSM_GENERIC_FAILURE -- Future reserved error status.
 */
...
/*============================================================================
                qapi_PSM_Client_Enter_Psm_With_Range
============================================================================*/
/**
 * @versiontable{2.0,2.45,
 * Modem\_Features 1.2.0  &  Introduced. @tblendline
 * }
 *  @brief Used by the application to indicate its intent to enter PSM mode
 *         with a time interval, i.e a start and end time about next wake up
 *
 *  @details The application must pass active_time in seconds, time interval for PSM,
 *           and whether the next wake up is for measurement purposes or access
 *           to the network.
 *           PSM Time interval is the time range within which the application needs
 *           device to be online. The actual absolute time when the device shall be
 *           online will be within this interval. PSM Daemon shall optimally calculate
 *           the exact time based upon votes of all the clients.
 *           PSM time can be accepted in either broken down format or in seconds.
 *           A PSM-aware application blocks PSM entry if either of Enter PSM APIs are not 
 *           called indefinitely.
 *
 *  @param[in] client_id Client ID obtained during registration.
 *  @param[in] psm_info  Pointer to a psm_info_type structure consisting of
 ...
 *
 *  @return  Returns QAPI_OK on success or a -ve error code on failure.	\n
 *  - QAPI_ERR_PSM_INVALID_INTERVAL -- End of an interval is before the start.
 *  - QAPI_ERR_PSM_WRONG_ARGUMENTS -- One or more of the arguments is NULL.
 *  - QAPI_ERR_PSM_APP_NOT_REGISTERED -- Invalid client ID.
 *  - QAPI_ERR_PSM_GENERIC_FAILURE -- Communication with the server failed.
 */
