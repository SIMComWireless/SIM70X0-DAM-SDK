...
uint32_t qapi_TSENS_Handler(uint32_t id, uint32_t a, uint32_t b, uint32_t c,
...
#define qapi_TSENS_Set_Thresholds(Handle, Sensor_Num,\
    Threshold_Type, Threshold_Degree, Threshold_CB, Context_Ptr, Threshold_payload)\
    ((qapi_Status_t) (_txm_module_system_call12)(TXM_QAPI_TSENS_SET_THRESHOLD,\
    (ULONG) Handle, (ULONG) Sensor_Num, (ULONG) Threshold_Type,\
    (ULONG) Threshold_Degree, (ULONG) Threshold_CB, (ULONG) Context_Ptr, (ULONG) qapi_custom_cb_TSENS_dispatcher,\
    (ULONG) Threshold_payload, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0))
...
*  @param[in]  payload          Pointer to the payload that will be filled and passed to the callback api incase a 
...
(
   ...
   QAPI_Tsens_Threshold_Cb_Type Threshold_CB,
   void *Context_Ptr,
   qapi_TSENS_CallbackPayloadType_t *payload
);
