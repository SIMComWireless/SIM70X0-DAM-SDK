...
#define TXM_QAPI_DAM_OPEN_CB              (CUSTOM_CB_QAPI_SIMCOM_BASE)
#define TXM_QAPI_SET_DTR_EVENT_CALLBACK_CB              (CUSTOM_CB_QAPI_SIMCOM_BASE + 1)
...
int qapi_DAM_user_cb_dispatcher(UINT cb_id, void *app_cb, UINT cb_param1, UINT cb_param2, UINT cb_param3)
{
    void(* simcom_visual_empty_cb_type)(void);
    void(* simcom_dtr_event_empty_cb_type)(uint32 dtr_status);
    
    if(cb_id == TXM_QAPI_DAM_OPEN_CB) // app_rx_cb
    {
          simcom_visual_empty_cb_type = (   void(*)(void))app_cb;
          (simcom_visual_empty_cb_type)();
    }
    else if (cb_id == TXM_QAPI_SET_DTR_EVENT_CALLBACK_CB)
    {
      simcom_dtr_event_empty_cb_type = (void(*)(uint32 dtr_status))app_cb;
      (simcom_dtr_event_empty_cb_type)(cb_param1);
    }
    return 0;
}
...
#define qapi_DAM_Visual_AT_Open(call_back)\
    ((unsigned short) (_txm_module_kernel_call_dispatcher)(TXM_QAPI_VISUAL_AT_OPEN, (ULONG) call_back,(ULONG)qapi_DAM_user_cb_dispatcher, (ULONG) 0))
...
#define qapi_DAM_Set_DTR_Event_Callback(call_back)\
    ((unsigned short) (_txm_module_kernel_call_dispatcher)(TXM_QAPI_SET_DTR_EVENT_CALLBACK, (ULONG) call_back,(ULONG) qapi_DAM_user_cb_dispatcher, (ULONG) 0))
