...
/******************************************************************************
  ...
  if using QMI client in DAM user space, need to call qmi_client_user_module_init once
  before using below Qmi client apis and while exiting application, should call 
  qmi_client_user_module_deinit from qmi_client_user.h header file.
...
$Header: //components/rel/core.tx/5.0/api/mproc/qmi_client.h#1 $
...
@note
  ...
  - Dependencies
    - If using from DAM user space application, make sure to call 
      qmi_client_user_module_init before using any QMI client apis
